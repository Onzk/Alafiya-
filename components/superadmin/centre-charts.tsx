...
export default function CentreCharts({ centreId, className = 'grid md:grid-cols-2 gap-4' }: { centreId: string; className?: string }) {
  const now = new Date()
  ...
  const fetchData = useCallback(async (ey: number, em: number, dy: number, dm: number) => {
    setLoading(true)
    try {
      const base = `/api/superadmin/centres/${centreId}/charts`
      const [resEnreg, resDossier] = await Promise.all([
        fetch(`${base}?year=${ey}&month=${em}`),
        fetch(`${base}?year=${dy}&month=${dm}`),
      ])
  ...
  return (
    <div className={className}>
      <ChartPanel
        ...
        subtitle="Nouvelles entrées par jour · ce centre"
        icon={FileText}
        iconBg="bg-brand/10 dark:bg-brand/15"
        iconColor="text-brand"
        gradientId={`c-gradEnreg-${centreId}`}
        strokeColor="#21c488"
        fillColor="#21c488"
      ...
      <ChartPanel
        ...
        subtitle="Nouveaux dossiers par jour · ce centre"
        icon={FolderOpen}
        iconBg="bg-blue-500/10 dark:bg-blue-400/15"
        iconColor="text-blue-600 dark:text-blue-300"
        gradientId={`c-gradDossier-${centreId}`}
        strokeColor="#3b82f6"
        fillColor="#3b82f6"
