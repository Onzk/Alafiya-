              ...
              {filtered.map((role, i) => (
                <li key={role.id} onClick={() => setViewTarget(role)} className={`dash-in delay-${[0,75,100,150,200,225,300][Math.min(i,6)]} grid grid-cols-[2fr_100px_70px_1fr_44px] items-center gap-4 px-5 py-3.5 border-b border-slate-50 dark:border-zinc-800/60 last:border-0 hover:bg-slate-50/60 dark:hover:bg-zinc-800/30 transition-colors cursor-pointer`}>
                  ...
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-lg w-fit ${role.creePar === 'CENTRE' ? 'bg-orange-50 dark:bg-orange-400/15 text-orange-600 dark:text-orange-400' : 'bg-brand/8 dark:bg-brand/12 text-brand'}`}>
                    {role.creePar === 'MINISTERE' ? 'Ministère' : 'Centre'}
                  </span>
                  {/* Personnel */}
            ...
            <DialogScrollableWrapper>
              <div className="space-y-4">
                <div className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-zinc-950 rounded-xl">
                  <div className={`h-12 w-12 rounded-xl flex items-center justify-center flex-shrink-0 ${viewTarget.creePar === 'CENTRE' ? 'bg-orange-50 dark:bg-orange-400/15' : 'bg-brand/10 dark:bg-brand/15'}`}>
                    <ShieldCheck className={`h-6 w-6 ${viewTarget.creePar === 'CENTRE' ? 'text-orange-600 dark:text-orange-400' : 'text-brand'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-bold text-slate-900 dark:text-white text-lg leading-tight">{viewTarget.nom}</p>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ${viewTarget.creePar === 'CENTRE' ? 'bg-orange-50 dark:bg-orange-400/15 text-orange-600 dark:text-orange-400' : 'bg-brand/8 dark:bg-brand/12 text-brand'}`}>
                        {viewTarget.creePar === 'MINISTERE' ? 'Ministère' : 'Centre'}
                      </span>
                    </div>
